...
#include "user_lib.h"
fp32 mouse_yaw = 0,mouse_pitch = 0;
...
void gimbal_behaviour_mode_set(gimbal_control_t *gimbal_mode_set)
{
    ...
    //根据云台行为状态机设置电机状态机
    if (gimbal_mode_set->gimbal_behaviour == GIMBAL_ZERO_FORCE)
    {
    ...
    else if (gimbal_mode_set->gimbal_behaviour == GIMBAL_INIT)
    {
        gimbal_mode_set->gimbal_yaw_motor.gimbal_motor_mode = GIMBAL_MOTOR_ENCONDE;
        gimbal_mode_set->gimbal_pitch_motor.gimbal_motor_mode = GIMBAL_MOTOR_ENCONDE;
    }
    else if (gimbal_mode_set->gimbal_behaviour == GIMBAL_ABSOLUTE_ANGLE)
    {
...
void gimbal_behaviour_control_set(fp32 *add_yaw, fp32 *add_pitch, gimbal_control_t *gimbal_control_set)
{
  ...
  static fp32 rc_add_yaw, rc_add_pit;
    ...
    rc_deadband_limit(gimbal_control_set->gimbal_rc_ctrl->rc.ch[PITCH_CHANNEL], pitch_channel, RC_DEADBAND);
		...
		//遥控器鼠标信号一阶低通滤波处理
//		mouse_yaw = LPF(&gimbal_control_set->yaw_lpf ,0.002,gimbal_control_set->gimbal_rc_ctrl->mouse.x,10);
//		mouse_pitch =LPF(&gimbal_control_set->pitch_lpf ,0.002,gimbal_control_set->gimbal_rc_ctrl->mouse.y,10);
		
    rc_add_yaw = (yaw_channel * YAW_RC_SEN +	             
		             mouse_yaw * YAW_MOUSE_SEN + 
		            gimbal_control_set->Q_mount - 
								 (gimbal_control_set->gimbal_rc_ctrl->key.v & GIMBAL_RIGHT_KEY)*0.008);
    rc_add_pit = (pitch_channel * PITCH_RC_SEN - mouse_pitch* PITCH_MOUSE_SEN);
...
static void gimbal_behavour_set(gimbal_control_t *gimbal_mode_set)
{
    ...
    //判断进入init状态机
    {
        ...
        if (last_gimbal_behaviour == GIMBAL_ZERO_FORCE && gimbal_mode_set->gimbal_behaviour != GIMBAL_ZERO_FORCE)
        {
            gimbal_mode_set->gimbal_behaviour = GIMBAL_INIT;
        }
        last_gimbal_behaviour = gimbal_mode_set->gimbal_behaviour;
    }
...
static void gimbal_absolute_angle_control(fp32 *yaw, fp32 *pitch, gimbal_control_t *gimbal_control_set)
{
    ...
    rc_deadband_limit(gimbal_control_set->gimbal_rc_ctrl->rc.ch[PITCH_CHANNEL], pitch_channel, RC_DEADBAND);
        ...
        *yaw = (-yaw_channel * YAW_RC_SEN -	             
		             mouse_yaw * YAW_MOUSE_SEN - 
		            gimbal_control_set->Q_mount + 
								 (gimbal_control_set->gimbal_rc_ctrl->key.v & GIMBAL_RIGHT_KEY)*0.000008);
		
		
    *pitch = pitch_channel * PITCH_RC_SEN + mouse_pitch * PITCH_MOUSE_SEN;
