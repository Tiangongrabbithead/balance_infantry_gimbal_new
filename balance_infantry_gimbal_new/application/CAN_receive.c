...
#define get_motor_measure(ptr, data)                                    \
    {                                                                   \
        ...
        (ptr)->temperate = (data)[6];  																	\
				if (fabs((ptr)->ecd - (ptr)->last_ecd) > 4096)										\
			{																																	\
...
/**
  ...
  * @retval         none
  */
void CAN_cmd_gimbal_command_0x712(Gimbal_order_t gimbal_order)
{
    uint32_t send_mail_box;
    gimbal_tx_message.StdId = 0x712;
    gimbal_tx_message.IDE = CAN_ID_STD;
    gimbal_tx_message.RTR = CAN_RTR_DATA;
    gimbal_tx_message.DLC = 0x08;
    memcpy((unsigned char*)gimbal_can_send_data,(unsigned char*)&gimbal_order,2*sizeof(fp32));

    HAL_CAN_AddTxMessage(&hcan2, &gimbal_tx_message, gimbal_can_send_data, &send_mail_box);
}
void CAN_cmd_gimbal_command_0x710(Gimbal_order_t gimbal_order)
{
    uint32_t send_mail_box;
    gimbal_tx_message.StdId = 0x710;
    gimbal_tx_message.IDE = CAN_ID_STD;
    gimbal_tx_message.RTR = CAN_RTR_DATA;
    gimbal_tx_message.DLC = 0x04;
    memcpy((unsigned char*)gimbal_can_send_data,(unsigned char*)&gimbal_order+8,4*sizeof(char));	
    HAL_CAN_AddTxMessage(&hcan2, &gimbal_tx_message, gimbal_can_send_data, &send_mail_box);
}

void CAN_cmd_gimbal_command_0x711(Gimbal_order_t gimbal_order)
{
    uint32_t send_mail_box;
    gimbal_tx_message.StdId = 0x711;
    gimbal_tx_message.IDE = CAN_ID_STD;
    gimbal_tx_message.RTR = CAN_RTR_DATA;
    gimbal_tx_message.DLC = 0x04;
    memcpy((unsigned char*)gimbal_can_send_data,(unsigned char*)&gimbal_order+12,8);	
    HAL_CAN_AddTxMessage(&hcan2, &gimbal_tx_message, gimbal_can_send_data, &send_mail_box);
}
/**
...
const motor_measure_t *get_motor_measure_point(int i)
{
